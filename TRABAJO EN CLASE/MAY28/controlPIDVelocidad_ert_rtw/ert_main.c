...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(controlPIDVelocidad_M, 0);
  ...
  controlPIDVelocidad_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(controlPIDVelocidad_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (controlPIDVelocidad_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(controlPIDVelocidad_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(controlPIDVelocidad_M);
